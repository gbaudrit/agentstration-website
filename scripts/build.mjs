...
<aside class="alpha-banner" aria-label="${lang === "fr" ? "Statut du projet" : "Project status"}">
  <div class="container alpha-banner-inner">
    <p><strong><i aria-hidden="true"></i>${lang === "fr" ? "Alpha publique · Développement actif" : "Public alpha · Actively developed"}</strong><span>${lang === "fr" ? "Les API, fonctionnalités et formats peuvent encore évoluer." : "APIs, features and formats may still change."}</span></p>
    <a href="${site.externalLinks.github}" target="_blank" rel="noreferrer">${lang === "fr" ? "Suivre sur GitHub" : "Follow on GitHub"}<span aria-hidden="true">↗</span></a>
  </div>
</aside>`;
}
...
function modeTopology(kind) {
  return `<span class="mode-topology ${kind}" aria-hidden="true"><i></i><i></i><i></i><i></i><b></b><b></b><b></b><b></b><b></b></span>`;
}

function flowModeFamily(family) {
  return `<article class="flow-family flow-family-${family.items.length > 3 ? "agents" : "structured"} reveal"><header><span>${family.label}</span><h3>${family.title}</h3><p>${family.body}</p>${family.runtime ? `<a class="flow-runtime-badge" href="${site.externalLinks.maf}" target="_blank" rel="noreferrer"><i aria-hidden="true"></i>${family.runtime}<b aria-hidden="true">↗</b></a>` : ""}</header><div class="flow-mode-list">${family.items.map(item => `<div class="flow-mode-card"><div><h4>${item[1]}</h4><p>${item[2]}</p></div>${modeTopology(item[0])}</div>`).join("")}</div></article>`;
}

function flowModesSection(lang, c, compact = false) {
  return `<section class="section flow-modes-section${compact ? " is-compact" : ""}" id="flow-modes"><div class="container"><div class="section-heading split"><div><p class="eyebrow">${c.flowModes.kicker}</p><h2>${c.flowModes.title}</h2></div><p>${c.flowModes.body}</p></div><div class="flow-family-grid">${flowModeFamily(c.flowModes.structured)}${flowModeFamily(c.flowModes.orchestration)}</div>${compact ? `<a class="arrow-link flow-modes-link" href="${pathFor(lang, "features")}#flow-modes">${c.flowModes.link}<span>→</span></a>` : `<p class="flow-modes-note">${c.flowModes.note}</p>`}</div></section>`;
}
...
function renderHome(lang, c) {
  ...
  <section class="section runtime-section"><div class="container runtime-grid"><div class="runtime-copy"><p class="eyebrow">${c.runtime.kicker}</p><h2>${c.runtime.title}</h2><p>${c.runtime.body}</p><div class="microsoft-stack"><span>${lang === "fr" ? "Socle technologique Microsoft" : "Microsoft technology foundation"}</span><div class="microsoft-stack-grid"><a href="${site.externalLinks.dotnet}" target="_blank" rel="noreferrer"><small>${lang === "fr" ? "PLATEFORME" : "PLATFORM"}</small><strong>.NET</strong><em>${lang === "fr" ? "Socle applicatif moderne et multiplateforme" : "Modern, cross-platform application foundation"}</em><b>${lang === "fr" ? "Site officiel" : "Official website"} ↗</b></a><a href="${site.externalLinks.maf}" target="_blank" rel="noreferrer"><small>${lang === "fr" ? "RUNTIME D’AGENTS" : "AGENT RUNTIME"}</small><strong>Microsoft Agent Framework</strong><em>${lang === "fr" ? "Runtime d’agents intégré actuellement" : "Currently integrated agent runtime"}</em><b>${lang === "fr" ? "Documentation officielle" : "Official documentation"} ↗</b></a></div></div></div><div class="runtime-diagram reveal" role="img" aria-label="Agentstration Runtime Plane connected to Microsoft Agent Framework"><div class="runtime-plane-card"><span>AGENTSTRATION</span><strong>Runtime Plane</strong><small>${lang === "fr" ? "Résolution · cycle de vie · observabilité" : "Resolution · lifecycle · observability"}</small></div><div class="runtime-bridge"><i></i><span>${lang === "fr" ? "ADAPTATEUR RUNTIME" : "RUNTIME ADAPTER"}</span><i></i></div><div class="maf-card"><span>MICROSOFT</span><strong>Agent Framework</strong><small>${lang === "fr" ? "Runtime d’agents intégré actuellement" : "Currently integrated agent runtime"}</small></div><div class="runtime-points">${c.runtime.points.map(x => `<div><p><strong>${x[0]}</strong>${x[1]}</p></div>`).join("")}</div></div></div></section>
